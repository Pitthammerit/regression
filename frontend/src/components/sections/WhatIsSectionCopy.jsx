...
export default function WhatIsSectionCopy({ debugMode = false }) {
  return (
    <SectionWrapper id="was-ist-copy" data-testid="whatis-section-copy">
      {/* Hidden: ensures text-semantic-accent CSS is generated by Tailwind JIT */}
      <span className="hidden text-semantic-accent" aria-hidden="true"></span>
        ...
        <div>
          {whatIs.body.map((para, i) => (
            <DebugLabel type="body" debugMode={debugMode} key={i}>
              <p className={`font-primary text-semantic-body leading-relaxed text-body ${i > 0 ? 'mt-6' : ''}`}>
                {para}
              </p>
            </DebugLabel>
          ))}
