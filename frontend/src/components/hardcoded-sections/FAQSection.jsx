          ...
          {faq.items.map((item, index) => (
            <div id={`faq-${index}`} key={index} className="border-t border-color-light" style={{ scrollMarginTop: 80 }}>
              <button
                ...
                className="w-full flex justify-between items-center py-6 text-left font-primary text-label hover:text-label/80 transition-colors"
              >
              ...
              >
                <p className="font-primary text-heading leading-relaxed whitespace-pre-line text-lg">
                  {item.answer}
                </p>
              </div>
            </div>
