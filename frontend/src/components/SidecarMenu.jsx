...
export default function SidecarMenu({ isOpen, onClose }) {
  ...
  return (
    <>
      ...
      {isBackdropVisible && (
        <div
          className={`fixed inset-0 bg-black/20 backdrop-blur-lg z-[60] transition-opacity ${
            shouldAnimateOut
              ? 'opacity-0 duration-[400ms] md:duration-[900ms] ease-out'  // Fade-Out: 400ms mobile, 900ms desktop
              : 'opacity-100'  // Instant on click, no fade-in
          }`}
          onClick={handleClose}
        />
      )}
      ...
      <div
        className="fixed top-0 right-0 h-full w-full 900:w-[380px] bg-brand-cream shadow-2xl z-[70] flex flex-col"
        style={{
